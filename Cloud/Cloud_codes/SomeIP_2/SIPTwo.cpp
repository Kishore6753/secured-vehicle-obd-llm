...
int main() {
    ...
    connection->Subscribe("SIPTwo_topic", Mqtt::QOS::AWS_MQTT_QOS_AT_LEAST_ONCE, onMessageReceived, onSubAck);

    // Main loop
    while (true) {
        // Step 1: Read the fifth line from payload_list.txt
        ...
        std::string skip1, skip2, skip3, skip4, jsonPath;
        ...
        if (!std::getline(listFile, jsonPath)) {  // read line 5
            std::cerr << "Failed to read fifth line from payload_list.txt" << std::endl;
            listFile.close();
            std::this_thread::sleep_for(std::chrono::seconds(5));
            continue;
        }
        ...
        if (jsonPath.empty()) {
            std::cerr << "Fifth line in payload_list.txt is empty" << std::endl;
            std::this_thread::sleep_for(std::chrono::seconds(5));
            continue;
        }
