...
int main() {
    ...
    connection->Subscribe("SIPOne_topic", Mqtt::QOS::AWS_MQTT_QOS_AT_LEAST_ONCE, onMessageReceived, onSubAck);

    // Main loop
    while (true) {
        // Step 1: Read the fourth line from payload_list.txt
        ...
        std::string skip1, skip2, skip3, jsonPath;
        std::getline(listFile, skip1);  // skip line 1
        std::getline(listFile, skip2);  // skip line 2
        std::getline(listFile, skip3);  // skip line 3

        if (!std::getline(listFile, jsonPath)) {  // read line 4
            std::cerr << "Failed to read fourth line from payload_list.txt" << std::endl;
            listFile.close();
            std::this_thread::sleep_for(std::chrono::seconds(5));
            continue;
        }
        ...
        if (jsonPath.empty()) {
            std::cerr << "Fourth line in payload_list.txt is empty" << std::endl;
            std::this_thread::sleep_for(std::chrono::seconds(5));
            continue;
        }
