...
int main() {
    ...
    connection->Subscribe("BL_topic", Mqtt::QOS::AWS_MQTT_QOS_AT_LEAST_ONCE, onMessageReceived, onSubAck);
    ...
    while (true) {
        ...
        std::string skip1, skip2, jsonPath;
        std::getline(listFile, skip1);  // Skip line 1
        std::getline(listFile, skip2);  // Skip line 2
        if (!std::getline(listFile, jsonPath)) {  // Read line 3
            std::cerr << "Failed to read third line from payload_list.txt" << std::endl;
            listFile.close();
            std::this_thread::sleep_for(std::chrono::seconds(5));
            continue;
        }
        ...
        if (jsonPath.empty()) {
            std::cerr << "Third line in payload_list.txt is empty" << std::endl;
            std::this_thread::sleep_for(std::chrono::seconds(5));
            continue;
        }
