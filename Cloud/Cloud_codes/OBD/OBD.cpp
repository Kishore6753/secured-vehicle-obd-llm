...
int main() {
    ...
    connection->Subscribe("obd_topic", Mqtt::QOS::AWS_MQTT_QOS_AT_LEAST_ONCE, onMessageReceived, onSubAck);

    // Main loop
    while (true) {
        // Step 1: Read path to JSON file from payload_list.txt
        ...
        std::string jsonPath;
        std::getline(listFile, jsonPath);
        listFile.close();

        if (jsonPath.empty()) {
            std::cerr << "payload_list.txt is empty or invalid" << std::endl;
            std::this_thread::sleep_for(std::chrono::seconds(5));
            continue;
        }
